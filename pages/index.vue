<template>
  <div>
    <section class="hero">
      <div class="hero__container">
        <div class="text">
          <h2>
            Shop unique home essentials,<br />
            all in one place
          </h2>
        </div>
        <div class="browse">
          <div class="categories">
            <h4>
              Browse Categories:
            </h4>
            <ul>
              <li>Furniture</li>
              <li>Outdoor</li>
              <li>Decor</li>
              <li>Storage</li>
              <li>Kitchen</li>
              <li>Appliances</li>
            </ul>
          </div>
        </div>
      </div>
    </section>
    <section class="flash">
      <div class="flash__left">
        <div class="title">
          Deals
        </div>
        <div class="image">
          <img src="~/assets/images/home/flash-1.png" class="flash_image" />
        </div>
      </div>
      <div class="flash__right">
        <div class="title">
          Sales
        </div>
        <div class="image">
          <img src="~/assets/images/home/sale.png" class="flash_image" />
        </div>
      </div>
    </section>
    <section class="featured__small">
      <div class="title">
        Featured Products
      </div>
      <div class="featured__box">
        <div class="box">
          <img src="~/assets/images/home/default_name(2).png" class="image" />
          <div class="box__title">
            <a href="/product">Simpsons Max Sofa House Chair</a>
          </div>
          <div class="box__amount">
            ₦80,000
          </div>
          <div class="box__rating">
            <star-rating
              :star-size="15"
              :show-rating="false"
              rating="4"
            ></star-rating>
          </div>
        </div>
        <div class="box">
          <img src="~/assets/images/home/default_name(3).png" class="image" />
          <div class="box__title">
            <a href="/product">Simpsons Max Sofa House Chair</a>
          </div>
          <div class="box__amount">
            ₦80,000
          </div>
          <div class="box__rating">
            <star-rating
              :star-size="15"
              :show-rating="false"
              rating="4"
            ></star-rating>
          </div>
        </div>
        <div class="box">
          <img src="~/assets/images/home/default_name(-2.png" class="image" />
          <div class="box__title">
            <a href="/product">Simpsons Max Sofa House Chair</a>
          </div>
          <div class="box__amount">
            ₦80,000
          </div>
          <div class="box__rating">
            <star-rating
              :star-size="15"
              :show-rating="false"
              rating="4"
            ></star-rating>
          </div>
        </div>
        <div class="box">
          <img src="~/assets/images/home/default_name(-3.png" class="image" />
          <div class="box__title">
            <a href="/product">Simpsons Max Sofa House Chair</a>
          </div>
          <div class="box__amount">
            ₦80,000
          </div>
          <div class="box__rating">
            <star-rating
              :star-size="15"
              :show-rating="false"
              rating="4"
            ></star-rating>
          </div>
        </div>
      </div>
    </section>
    <section class="featured__large">
      <div class="title">
        Featured Products
      </div>
      <div class="featured__box">
        <div class="box" @click="gotoLink()">
          <div class="box__title">
            FURNITURE
          </div>
          <img src="~/assets/images/home/armchair.png" class="image" />
          <div class="image__container">
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-4.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-5.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-6.png"
                class="image"
              />
            </div>
          </div>
        </div>
        <div class="box" @click="gotoLink()">
          <div class="box__title">
            OUTDOOR
          </div>
          <img src="~/assets/images/home/armchair.png" class="image" />
          <div class="image__container">
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-4.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-5.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-6.png"
                class="image"
              />
            </div>
          </div>
        </div>
        <div class="box" @click="gotoLink()">
          <div class="box__title">
            DECOR
          </div>
          <img src="~/assets/images/home/armchair.png" class="image" />
          <div class="image__container">
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-4.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-5.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-6.png"
                class="image"
              />
            </div>
          </div>
        </div>
        <div class="box" @click="gotoLink()">
          <div class="box__title">
            STORAGE
          </div>
          <img src="~/assets/images/home/armchair.png" class="image" />
          <div class="image__container">
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-4.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-5.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-6.png"
                class="image"
              />
            </div>
          </div>
        </div>
        <div class="box" @click="gotoLink()">
          <div class="box__title">
            KITCHEN
          </div>
          <img src="~/assets/images/home/armchair.png" class="image" />
          <div class="image__container">
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-4.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-5.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-6.png"
                class="image"
              />
            </div>
          </div>
        </div>
        <div class="box" @click="gotoLink()">
          <div class="box__title">
            APPLIANCES
          </div>
          <img src="~/assets/images/home/armchair.png" class="image" />
          <div class="image__container">
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-4.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-5.png"
                class="image"
              />
            </div>
            <div class="image__box">
              <img
                src="~/assets/images/home/default_name(-6.png"
                class="image"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="slide">
      <div class="content">
        <h3>The best room furniture</h3>
        <p>
          Browse and shop furniture<br />
          for your home
        </p>
        <button>Step Inside</button>
      </div>
      <div class="image">
        <img src="~/assets/images/home/room.png" class="slide__image" />
      </div>
    </section>
    <section class="newsletter">
      <h5>Be in the know</h5>
      <p>Get recommendations, updates, promotions and more</p>
      <div class="form__control">
        <input type="text" />
        <button>Sign up</button>
      </div>
    </section>
  </div>
</template>

<script>
export default {
  components: {
    // StarRating
  },
  methods: {
    gotoLink() {
      window.location = "/products";
    }
  }
};
</script>

<style></style>
